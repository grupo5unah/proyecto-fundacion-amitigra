...
//INSERTAR PARA CAMPING
    ...
    if(!empty($descripcionC) || !empty($cantidad_adultosC) ||!empty($cantidad_ninosC) || !empty($cantidad_articuloC) || !empty($totalC) ||!empty($nombre_cliente) ||!empty($idCliente)
    ||!empty($fecha_reservacion)||!empty($fecha_entrada)||!empty($fecha_salida)||!empty($identidad)||!empty($telefono)||!empty($nacionalidad)
    ||!empty($id_usuario)||!empty($usuario_actual)){
        ...
        try{
            ...
            estado_eliminado,creado_por, fecha_creacion, modificado_por, fecha_modificacion) 
            VALUES (?,?,?,?,?,?,?,?,?,?);");
            $inserta->bind_param('sssiiissss', $fecha_reservacion, $entrar,$salir,$idCliente,$id_usuario,$estado_eliminado,$usuario_actual,$fecha,$usuario_actual,$fecha);
            $inserta->execute();
            ...
            if($capturar_reserva->affected_rows){
                ...
                if($existe_reservacion){
                    ...
                    total_pago,estado_eliminado,creado_por, fecha_creacion, modificado_por, fecha_modificacion) 
                    VALUES (?,?,?,?,?,?,?,?,?,?);");
                    $insert->bind_param('iiiiiissss', $idr,$descripcionC,$cantidad_adultosC, $cantidad_ninosC, $cantidad_articuloC,$totalC,$estado_eliminado,$usuario_actual,$fecha,$usuario_actual,$fecha);
                    $insert->execute();

                    //se captura el id de la tabla de habitacion servicio para cambiarle el estado
                    $captura_ha = $conn->prepare("SELECT habitacion_id FROM tbl_detalle_reservacion
                    WHERE habitacion_id = ?;");
                    $captura_ha->bind_Param("s", $descripcionC);
                    $captura_ha->execute();
                    $captura_ha->bind_result($idA);
