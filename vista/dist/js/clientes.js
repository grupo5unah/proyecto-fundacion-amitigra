$(document).ready(function(){
    ...
    //BUSCAR CLIENTE   
    $('#ident').blur(async function () {
        console.log(this.value);
        if(this.value.length > 0 ){
            try{
                const resp = await axios(`./controlador/ctr.mantClientes.php?action=obtenerCliente&cliente=${this.value}`);
                const data = resp.data;
                ...
                //    console.log(data.objeto[0]);
                    ...
                    return swal('Este Cliente ya existe ');
                }
            }catch(err){
    ...
    $('.btnEditarCliente').on('click', function() {
        ...
        $('.btnEditarBD').on('click', async function() {
            ...
            formData.append('cliente',$("#nombre_cliente").val());
